...
| [0020](0020-unified-calibre-ingest-hardlink-delivery.md) | Unify ebook ingest on direct `calibredb add`; remove CWA ingest (CWASink/slim/push_cwa); per-sink sidecars; `delivery_mode` hardlink (os.link at delivery for ABS/folder, post-import relink for calibredb); lean pre-relink-gate safety; slim image frozen as LTS |
| [0021](0021-roster-gates-discovery-author-creation.md) | The **roster** (allow-listed OR owns a book in-library, any position) gates discovery author creation *and* scan eligibility; library sync unchanged and surfaces non-roster authors for review; scan skips silent. Amends 0014 |
