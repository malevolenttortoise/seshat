...
- **Metadata cache** — a local store of source-fetched author/book data (per source), populated by a background **cache worker**, consulted at scan time in place of a live source call. Decouples user-facing scan latency from source-side rate-limit / throughput pain. One cache DB file per source (`metadata_cache_<source>.db`).
- **Cache worker** — the background paced process that fills a source's metadata cache. One per source. The Amazon worker (v2.21.0) primarily solves a *cooldown* problem (Akamai 200–400/day cap); a Goodreads worker primarily solves a *throughput* problem (5–7s/book detail-fetch pacing, soft-degradable, no hard cap).
- **First-fill** — the cold-start period after enabling a new source's metadata cache, during which the worker is initially populating rows. User-facing scans may return partial data with progress badges until the worker catches up.
- **List-page cache** — caching only an author's book-ID inventory from the source's list page. Smaller storage; instant scan dedup against existing local rows; no read-time filtering on detail-level facts (language, format, edition).
- **Detail cache** — caching full per-book detail data alongside list pages. Enables read-time filtering on language/format/edition without live HTTP, eliminating wasted detail fetches on books that would fail filters post-fetch.
- **Budget exhaust** — Goodreads-specific failure mode where the per-author wall-clock budget (tier-scaled by list-page book count: ≥200 → 40min, ≥100 → 30min, else 25min) hits its cap before all detail fetches complete; the scan finalizes with whatever was processed. The `[goodreads] giving up on '<author>' — processed N/M books` log line is the operator signal.
...
- **Source author ID** — a source's own stable identifier for an author (Goodreads `/author/show/<id>`, Hardcover author id, Amazon/Audnexus ASIN, OpenLibrary key). Stored per source on the per-library `authors` row (`{source}_id` columns). A stronger identity signal than a name string, since it survives name-spelling drift.
- **ID-aware matching** — resolving author identity by **source author ID first**, then falling back to name/normalized/fuzzy. Applies at two layers: per-library author resolution and cross-library **person** consolidation (matched *through* `author_links` — the person inherits the IDs on its linked authors). Closes the split-person gap where one author lands on two persons because the names differ ("Robert Heinlein" vs "Robert A. Heinlein") even though their author rows share a source ID (see [ADR-0015](docs/adr/0015-source-id-aware-author-identity.md)).
- **Source-ID conflict** — when a scan's captured author ID disagrees with an ID already on file: the incoming ID matched no row, but the incoming *name* matched a row that already holds a *different* ID for that source. A genuine identity ambiguity (a wrong name-match, or a source that split one author across profiles). Recorded rather than auto-resolved, and surfaced for operator review; the on-file ID is never silently overwritten (see [ADR-0015](docs/adr/0015-source-id-aware-author-identity.md)).
- **Mirror** — write-through that propagates a canonical value (bio, image) to per-library author siblings and the canonical `persons` row. Two helpers with deliberately different semantics: `mirror_bio` **COALESCE-fills** siblings (preserve per-library values when populated; only fill NULLs) because bio plausibly differs by source/library; `mirror_image_url` does **canonical-overwrite-all** (every sibling + `persons` carries the same `(image_url, image_url_source)` tuple in lockstep) because a person's face is invariant across libraries — sibling drift would render the same person as different faces depending on which library a page loaded from (see [ADR-0016](docs/adr/0016-author-image-source-rank-and-mirror.md)).
- **Image source rank** — locked priority order (lower number = higher priority) for resolving cross-source contention on the single `authors.image_url` column: `amazon` (1) > `goodreads` (2) > `hardcover` (3) > `audible` (4). Recorded alongside the value in `image_url_source` (on both `persons` and per-library `authors`). The **scanned-author** write path overwrites only when `incoming_rank ≤ on_file_rank`; the **co-author** write path is strict fill-if-empty regardless of rank (a low-confidence byline never upgrades cross-source). Hardcover and audible are reserved-but-inert until their parsers emit images (see [ADR-0016](docs/adr/0016-author-image-source-rank-and-mirror.md)).
...
- **Delivery mode** — the global `copy | hardlink` policy for how a sink places the **primary** file (sidecars are always copied). `copy` is the default and current behaviour; `hardlink` shares the seeding torrent file's inode with the library file to avoid duplicating bytes. The mechanism differs by sink: read-in-place sinks (ABS, folder) `os.link` at delivery; the calibredb path uses a **hardlink relink** after import (see [ADR-0020](docs/adr/0020-unified-calibre-ingest-hardlink-delivery.md)).
- **Hardlink relink** — the calibredb-path hardlink step: because `calibredb add` copies the book into Calibre's managed tree, the resulting library file is *post-import* atomically replaced with a hardlink to the seeding inode — but only after a **byte-identity gate** (library bytes == seed bytes) and a same-filesystem check pass; otherwise the plain copy is kept. The book file is delivered unmodified (metadata rides `calibredb` flags, not the file), which is what makes the two files identical and relinkable (see [ADR-0020](docs/adr/0020-unified-calibre-ingest-hardlink-delivery.md)).
