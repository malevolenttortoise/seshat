...
- **Live credentials are resolved per-call, never frozen at startup — and there is exactly ONE authority per credential.** The MAM cookie lives in `app.mam.cookie`; read it via `DispatcherDeps.live_mam_token()` (or `get_active_token()`), never from a snapshot field. Two failure modes, both shipped in v3.10.0 and fixed in v3.10.1: (a) the long-lived background loops capture one `DispatcherDeps` at startup (`main.py`'s `deps_for_loops`) and keep it for the process lifetime, so a credential save that only rebuilds `state.dispatcher` never reaches them; (b) `app/discovery/sources/mam.py` kept a *parallel* `_current_token` with the precedence inverted (`_current_token or token`), which nothing re-seeded on save, so it pinned a dead session. Combined symptom: MAM Status green + upload-credit purchases fine, while every search 403'd and every grab 401'd until the container was restarted. Resolve through the single authority at call time; if you add a second cache for a credential, you have re-created this bug.
